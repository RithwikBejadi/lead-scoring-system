      ...
      [{
        $set: {
          ...
          leadStage: {
            $switch: {
              branches: [
                { case: { $gte: [{ $multiply: ["$currentScore", DECAY_FACTOR] }, 60] }, then: "qualified" },
                { case: { $gte: [{ $multiply: ["$currentScore", DECAY_FACTOR] }, 31] }, then: "hot" },
                { case: { $gte: [{ $multiply: ["$currentScore", DECAY_FACTOR] }, 11] }, then: "warm" }
              ],
              default: "cold"
            }
          }
